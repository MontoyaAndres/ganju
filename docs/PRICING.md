...
**The fourth meter now exists in test mode, and the chain behind it is exercised** — `scripts/verify-tool-call-metering.mjs --live-stripe` reports a real overage through the cron's own function and reads the aggregate back, and the deployed worker's checkout session carries the price. **The fourth meter**: `ganju_custom_tool_calls`, same shape as the rest — aggregation `sum`, value key `value`, customer key `stripe_customer_id` — with a metered price on the Pro product at **$0.005 per package of 1,000 calls**, which is $5 per million.
...
**Live mode is untouched, and nothing carries over from test.** As of 5 Sep the blocker is not the checklist below but the Stripe account itself: it is not through **activation**, so live mode has no keys to create anything with. That is a business step — entity type, tax id, payout account, identity verification — and it gates payment only. Production runs the Free tier fine without it, because `createStripe` answers `null` on a missing key, the hourly sweep returns early, and `getPlan` / `getStatus` never touch Stripe.

Two things follow while it waits. **Leave `STRIPE_SECRET_KEY` unset** rather than filling it with a placeholder — unset is the handled path, a bogus key builds a real client that throws every hour. And **granting a paid plan by hand means `plan = 'PRO'` with an entitled status, never `ENTERPRISE`**: the two differ in exactly one field, `toolCallHardCap`, which Enterprise sets to `null` — so Enterprise gives a hand-granted customer nothing extra while removing the only monthly ceiling on their compute.

**One trap for the day it is switched on.** `meterOrganization` reports `current − reported` and advances the `reported_*` marks only when an event lands; with no Stripe client it returns before both, so those marks sit at 0 while usage accrues. The first run after Stripe is configured would report the *entire* period, including anything already invoiced out of band. Enable it just after a period rollover, or set each `reported_*` column to its current counterpart first.

Once the account is activated, live mode needs, in order:
