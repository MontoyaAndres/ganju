...
~~**A redeploy can smoke-test against the previous edition.**~~ — fixed. Uploading a script into the dispatch namespace is not read-your-writes: dispatching to the name immediately afterwards can reach the previous edition. Found by deploying over a real artifact, and invisible to every probe before it, because they all scaffold a throwaway artifact — a new script name has no previous edition to serve.

The loud half was merely annoying: a deploy that renamed a tool failed with "the bundle does not export …" and passed on retry. The dangerous half was silent. A deploy that renamed *nothing* asked the old script "do you export these names", got yes, and published — leaving the customer running code they did not deploy until propagation caught up. `ganju test` had the same flaw more sharply, since every test reuses one preview script name, so a test could show the previous run's behaviour and read as "my edit did nothing".

Tool names could never have caught that, so the script now says which edition it is. `GANJU_SCRIPT_VERSION` is injected as a plain-text binding at upload, the SDK's health probe echoes it, and `smokeTestCustomCodeScript` polls until the edition answering is the one it just wrote — capped at `CUSTOM_CODE_SMOKE_TIMEOUT_MS` (20s), after which it fails with a 503 saying *this is propagation, not your code*. The preview path makes the same wait before a test runs.

**The marker is the digest of the bytes, not the version id.** A draft is re-uploaded in place — `PUT …/bundle` overwrites `sourceKey` on the row it was given — so a version id is the same string across every edit of the same draft. On the publish path that distinction rarely shows, since a publish usually carries a new version. On the test path it is the whole case: every test of one draft uploads to the same preview script name *and* would have carried the same version id, so the wait would have matched the previous run's script on the first probe and reported the previous run's behaviour as this one's — the exact "my edit did nothing" it was added to prevent. `deployCustomCodeScript` hashes what it uploads and returns it, so the value waited on is the bytes that were sent rather than anything a caller passed alongside them. Two uploads of identical bytes do share a marker, which is the one case where not telling them apart is the right answer.

Four details worth keeping:

- **A script with no marker falls back to the name check, but only at the deadline.** Anything deployed before this answers `null` forever, and waiting on it indefinitely would turn every publish over one into a timeout. It cannot be believed on the first probe either: the upload being verified always carries a marker, so an unmarked answer is by definition the previous edition — the very script the wait exists to see past. So it waits like any other stale edition, and the weaker check is what the deadline does instead of failing, which costs an existing artifact's first publish a second or two and nothing after that.
- **"Nothing is answering yet" waits the same as "the wrong edition is answering".** They are the same race — on a first deploy, propagation means the namespace has no script to dispatch to at all — so a dispatch failure is a value the loop retries, not an exception that ends the publish. Anything that *does* answer and answers wrongly still fails immediately: something is serving, and it is not an SDK script.
- **The timeout carries its own status.** `handleError` replaces an unmatched message with "Internal Server Error", and here the message *is* the value — it tells the reader their code is fine. `HandleErrorResult` gained 503 for it, which is the only status an error sets on itself here about timing rather than about the request. Matched, in passing: the export-mismatch error was itself an opaque 500 for the same reason, and now reads as a 400.
- **It costs a few seconds.** Publishing went from instant to ~4–6s on a namespace that already holds the script. That is the correct trade: the alternative is a publish that returns quickly and is sometimes a lie.

Verified by [scripts/probe-redeploy.mjs](../scripts/probe-redeploy.mjs) — 20 checks — which deploys repeatedly to one artifact and asserts, against the real dispatch namespace, that the second deploy's code is what gets served when the tool name did not change. It also confirms the wait did not weaken the check it wraps: a bundle that genuinely fails to export what it declares is still refused, now as a 400, the failed publish leaves `activeVersionId` where it was, the previously deployed script is restored, and a correct publish immediately afterwards still works — which is what proves the restore leaves the marker describing the edition that is actually serving. The last section covers the test path on its own terms: one draft, edited between two runs, has to report the edit rather than the run before it.

**A rejected publish used to take the working tools with it — also fixed.** Publishing has to upload before it can check, because the only way to ask a script what it exports is to deploy it and call it. So a validation failure arrived after the artifact's one script had already been replaced. `activeVersionId` stayed put, which is correct, and that is exactly what left the two out of step: the MCP server went on advertising the previous version's tool names over the bundle that had just been rejected, and every one of them answered *"this script does not export …"*. A typo in a tool name took down tools that were working.

Refusing a publish is only correct if what was working still works, so the failure path now puts the previous version's bundle back. With no previous version — a first publish that fails — the script is removed instead, because that is the state `activeVersionId: null` already describes, and an artifact advertising nothing while holding a script nobody asked for is not a state worth keeping.

It is best-effort by design. The caller is already throwing, and the error it carries — what is wrong with the bundle — is the one the author can act on; a restore that fails is logged rather than allowed to replace that with a message about our plumbing. The redeploy stamps the restored bytes with their own digest, so the marker keeps describing the edition that is really serving and the next publish waits on something that arrives.

The test endpoint deploys before it waits too, and its cleanup now covers both: the smoke test moved inside the `try` whose `finally` removes the preview script. Timing out is the likeliest way that endpoint fails, and a preview script left behind by the path that failed early is the same leak as one left behind by a failed delete.
