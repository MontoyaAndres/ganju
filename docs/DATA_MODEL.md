...
- **organizationLlm** holds reusable LLM connections (Anthropic / OpenAI / OpenAI-compatible / Google) that channels reference. An `openai-compatible` row carries a required `baseUrl` and reaches any service speaking that protocol — see [MODELS.md](MODELS.md).
...
`artifactResource` carries `type` (`static` | `template`), `sourceType` (`FILE` | `WEBSITE` | `GOOGLE_DRIVE_FOLDER` | `ONE_DRIVE_FOLDER` | `CUSTOM_CODE`), `status` (`PENDING`/`COMPLETED`/`FAILED`), mime type, and either inline `content`, an R2 `fileKey`, or a crawl/sync config. Folders and crawls are hierarchical via `parentResourceId` (self-reference) with a `childResourceCount`. A sync root — a website seed, a Drive/OneDrive folder, or a file imported from either — carries `syncInterval` (`off` | `daily` | `weekly`, default `weekly`) and `syncStartedAt`, which the hourly `runResourceSync` job compares against the interval for plans whose `autoSyncIntervals` include it; `metadata.lastSyncedAt` records when a sync last finished. Crawled pages keep a `metadata.contentHash` of their extracted text so a re-crawl re-embeds only pages that changed.
